/**
 * Representment letter drafting.
 *
 * ## Provenance, stated plainly
 *
 * There are two draft sources and they are never conflated:
 *
 *   MODEL     Generated by Claude via the Anthropic API. Requires
 *             ANTHROPIC_API_KEY and the --live flag. Cached to
 *             data/llm-cache/ keyed on a hash of the prompt.
 *
 *   TEMPLATE  A deterministic letter assembled from the evidence assessment.
 *             No model involved.
 *
 * Every draft carries its `source`, and the CLI prints it. Shipping
 * template output while implying a model wrote it would be the single most
 * dishonest thing this repo could do, and it is exactly the kind of thing a
 * reviewer probes for. The committed cache is empty until someone runs with a
 * key, so what a reviewer sees offline is honestly labelled TEMPLATE.
 *
 * ## Why cache at all
 *
 * The repo must run end to end with no API key. Keying the cache on a hash of
 * the prompt means regeneration is a no-op when nothing changed, and any edit
 * to the prompt or the case invalidates exactly the entries it should.
 */
...
export interface RepresentmentDraft {
  ...
  /** Present only when a model produced the text, directly or from cache. */
...
export function buildPrompt(
  ...
  // The prompt deliberately forbids referencing evidence the merchant does not
  // hold. A representment that cites a document the acquirer then cannot find
  // is worse than no representment at all.
  return [
...
function readCache(hash: string): CacheEntry | null {
  ...
  try {
    ...
    // A corrupt cache entry should regenerate, not crash the run.
    return null;
  }
}
...
/**
 * Deterministic letter, assembled from the assessment. Honestly labelled.
 *
 * It is genuinely usable rather than a placeholder: it cites the right
 * artifacts and makes the right argument. It simply is not model output, and
 * the CLI says so every time it prints one.
 */
...
export interface DraftOptions {
  /** Call the API. Requires ANTHROPIC_API_KEY. Off by default. */
  readonly live?: boolean;
}

/**
 * Draft a representment.
 *
 * Order of preference: cache, then a live model call if explicitly enabled,
 * then the template. Never silently calls the network -- a benchmark repo that
 * makes surprise API calls is a repo a reviewer stops trusting.
 */
...
/**
 * Minimal Anthropic Messages API call over fetch.
 *
 * Deliberately not the SDK: one dependency fewer, and the repo must install
 * and run for a reviewer who will never set an API key. Node 22 has fetch.
 */
