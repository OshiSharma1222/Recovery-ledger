...
function sweep(runs: number): void {
  const rows: {
    seed: string;
    liftVsFixed: number;
    liftVsTimed: number;
  ...
  for (let i = 1; i <= runs; i++) {
    const seed = `sweep-${String(i).padStart(2, "0")}`;
    const report = runBenchmark({
      trainSeed: `${seed}:train`,
      testSeed: `${seed}:test`,
    });
    ...
    const row = {
      seed,
      liftVsFixed: get("B3").recoveredPaise / get("B1").recoveredPaise - 1,
      liftVsTimed: get("B3").recoveredPaise / get("B2T").recoveredPaise - 1,
      ceiling: get("B3").ceilingCapture ?? 0,
    };
    rows.push(row);
    console.log(
      `  ${seed}   vs fixed ${pct(row.liftVsFixed).padStart(7)}   vs timed ${pct(row.liftVsTimed).padStart(7)}   of ceiling ${pct(row.ceiling)}`,
    );
  }
  ...
  const fixed = stat(rows.map((r) => r.liftVsFixed));
  const timed = stat(rows.map((r) => r.liftVsTimed));
  const ceil = stat(rows.map((r) => r.ceiling));

  console.log("");
  console.log(`  Across ${runs} unseen seeds (none of them the committed one):`);
  console.log(
    `  lift vs fixed schedule   min ${pct(fixed.min)}   median ${pct(fixed.median)}   max ${pct(fixed.max)}`,
  );
  console.log(
    `  lift vs smart timing     min ${pct(timed.min)}   median ${pct(timed.median)}   max ${pct(timed.max)}`,
  );
  console.log(
    `  share of oracle ceiling  min ${pct(ceil.min)}   median ${pct(ceil.median)}   max ${pct(ceil.max)}`,
  );
  ...
  const md = [
    ...
    `this runs ${runs} different ones. If the headline only held on a lucky seed,`,
    ...
    ...rows.map(
      (r) =>
        `| \`${r.seed}\` | ${pct(r.liftVsFixed)} | ${pct(r.liftVsTimed)} | ${pct(r.ceiling)} |`,
    ),
    "",
    "| | min | median | max |",
    "|---|---:|---:|---:|",
    `| lift vs fixed schedule | ${pct(fixed.min)} | ${pct(fixed.median)} | ${pct(fixed.max)} |`,
    `| lift vs smart timing | ${pct(timed.min)} | ${pct(timed.median)} | ${pct(timed.max)} |`,
    `| share of oracle ceiling | ${pct(ceil.min)} | ${pct(ceil.median)} | ${pct(ceil.max)} |`,
    "",
  ...
  writeFileSync("results/robustness.md", md, "utf8");
  console.log("");
  console.log("  Written to results/robustness.md");
  console.log("");
}
...
function main(): void {
  ...
  if (sweepArg) {
    console.log("");
    console.log("  RECOVERY LEDGER -- robustness sweep");
    console.log("  " + "=".repeat(78));
    sweep(Number(sweepArg));
    return;
  }
